...
finish() {
  ...
  if [ "$result" -ne 0 ]; then
    printf 'Release failed: stage=%s backup=%s\n' "$stage" "$backup_path" >&2
    if [ "$replaced" = 1 ] && [ -n "$previous" ]; then
      APP_IMAGE=$previous
      export APP_IMAGE
      if compose up -d --wait --wait-timeout 120 app >/dev/null 2>&1; then
        printf '%s\n' "$previous" > "$state/current-image"
        echo 'Previous application image restored; database was not reverted.' >&2
      else
        echo 'Application rollback failed; operator recovery required. Database was not reverted.' >&2
      fi
    elif [ "$replaced" = 1 ]; then
      compose stop app >/dev/null 2>&1 || true
      echo 'No previous image; failed initial staging application stopped.' >&2
    fi
    printf 'failed %s backup=%s\n' "$stage" "$backup_path" > "$state/result"
  fi
  exit "$result"
}
trap finish EXIT
