...
const tool = fileURLToPath(
  new URL('../../tools/import-json-data.mjs', import.meta.url),
);
...
function run({ source, database }, args = []) {
  return spawnSync(
    process.execPath,
    [tool, '--source', source, '--database', database, ...args],
    { encoding: 'utf8' },
  );
}
...
test('import is idempotent by record ID, reconciles counts, and leaves source bytes untouched', async (t) => {
  ...
  assert.deepEqual(JSON.parse(second.stdout), {
    dryRun: false,
    inquiries: { source: 2, imported: 0, skipped: 2 },
    events: { source: 3, imported: 0, skipped: 3 },
  });
  const result = readData(files.database);
  assert.equal(result.inquiries.length, 2);
  assert.equal(result.events.length, 3);
  assert.deepEqual(
    result.inquiries.find((row) => row.id === 'a'),
    inquiry('a'),
  );
  ...
  assert.doesNotMatch(
    ...
    /Synthetic|00000000000|synthetic-visitor/,
  );
});
...
test('invalid source records fail without printing values or importing any records', async (t) => {
  const files = await fixture(t, {
    version: 1,
    inquiries: [inquiry('a')],
    events: [event('a'), event('b', { eventType: 'secret-personal-field' })],
  });
  ...
  assert.doesNotMatch(
    ...
    /secret-personal-field|Synthetic|00000000000/,
  );
  await assert.rejects(stat(files.database), { code: 'ENOENT' });
});
...
test('a destination write failure rolls back inquiries and earlier events in the import', async (t) => {
  ...
  migrate(db);
  db.exec(
    "CREATE TRIGGER reject_event BEFORE INSERT ON analytics_events WHEN NEW.id = 'b' BEGIN SELECT RAISE(ABORT, 'private-trigger-message'); END",
  );
  db.close();
  const result = run(files);
  assert.notEqual(result.status, 0);
  ...
  assert.doesNotMatch(
    result.stdout + result.stderr,
    /private-trigger-message|Synthetic|00000000000/,
  );
...
test('duplicate source IDs reconcile as skipped and existing records are never overwritten', async (t) => {
  const files = await fixture(t, {
    version: 1,
    inquiries: [inquiry('a'), inquiry('a', { status: 'Won' })],
    events: [event('a'), event('a')],
  });
  const result = run(files);
  assert.equal(result.status, 0, result.stderr);
  assert.deepEqual(JSON.parse(result.stdout).inquiries, {
    source: 2,
    imported: 1,
    skipped: 1,
  });
  ...
  const files = await fixture(t, {
    ...
    inquiries: [
      inquiry('a', {
        ...
        source: 'private',
        sourcePage: '/private',
        sourceSection: 'private',
        referrer: 'private',
      }),
    ],
    events: [event('a', { phone: 'private', ip: 'private' })],
  });
  assert.equal(run(files).status, 0);
  const result = readData(files.database);
...
test('malformed structures, invalid dates, missing consent, unsafe paths and CLI flags fail safely', async (t) => {
  for (const data of [
    null,
    {},
    { version: 2, inquiries: [], events: [] },
    { version: 1, inquiries: [], events: {} },
    {
      version: 1,
      inquiries: [inquiry('a', { createdAt: 'invalid' })],
      events: [],
    },
    {
      version: 1,
      inquiries: [],
      events: [event('a', { analyticsConsentAt: undefined })],
    },
  ]) {
