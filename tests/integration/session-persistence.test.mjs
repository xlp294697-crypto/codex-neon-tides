...
test('signed sessions survive process restart, expire during reads, and persist only keyed hashes', async (t) => {
  ...
  for (const flag of ['HttpOnly', 'SameSite=Strict', 'Path=/', 'Secure']) assert.equal(login.setCookie.includes(flag), true);
  await fixture.restart();
  const result = await fixture.request('/api/session', { headers: { cookie: login.cookie } });
  ...
  const hash = createHmac('sha256', fixture.config.sessionSecret).update(token).digest('hex');
  ...
  for (const secret of [token, login.csrfToken, login.cookie, fixture.config.sessionSecret]) {
    assert.equal(JSON.stringify(rows).includes(secret), false, 'Stored session must contain no credentials');
  }
  db.prepare('UPDATE admin_sessions SET expires_at = ?').run(Date.now());
  assert.equal((await fixture.request('/api/session', { headers: { cookie: login.cookie } })).response.status, 401);
...
test('logout remains revoked after process restart', async (t) => {
  const fixture = await httpFixture(t, { subprocess: true });
  const login = await fixture.login();
  assert.equal((await fixture.request('/api/logout', { method: 'POST', headers: { cookie: login.cookie, 'x-csrf-token': login.csrfToken } })).response.status, 200);
  await fixture.restart();
  assert.equal((await fixture.request('/api/session', { headers: { cookie: login.cookie } })).response.status, 401);
});
