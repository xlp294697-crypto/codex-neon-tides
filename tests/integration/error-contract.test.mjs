...
function contract(result, status, code, secrets = []) {
  ...
  for (const value of [
    ...
    assert.equal(
      ...
      false,
      'Error envelope leaked internal details',
    );
  }
}
...
test('validation, authentication, CSRF and rate-limit errors have stable safe codes and unique request IDs', async (t) => {
  ...
  const cases = [
    ...
    [
      '/api/inquiries',
      post({ ...inquiry, phone: 'invalid' }),
      422,
      'INVALID_INQUIRY',
    ],
    [
      '/api/inquiries',
      post({ ...inquiry, privacyConsent: false }),
      422,
  ...
  for (let i = 0; i < 5; i++)
    contract(
      await fixture.request(
        '/api/login',
        post({ password: 'synthetic-wrong' }),
      ),
      401,
      'INVALID_CREDENTIALS',
    );
  contract(
    await fixture.request('/api/login', post({ password: 'synthetic-wrong' })),
    429,
    'RATE_LIMITED',
  );
  assert.equal(ids.size, cases.length);
  const success = await fixture.request('/api/catalog', {
...
test('unexpected SQLite failure hides internal details and logs only request ID and category', async (t) => {
  ...
  db.exec(
    "CREATE TRIGGER reject_inquiry BEFORE INSERT ON inquiries BEGIN SELECT RAISE(ABORT, 'SQLITE synthetic-sensitive-path password cookie'); END",
  );
  const logs = [];
  t.mock.method(console, 'error', (...args) => logs.push(args.join(' ')));
  const result = await fixture.request('/api/inquiries', post(inquiry));
  contract(result, 500, 'INTERNAL_ERROR', [
    fixture.config.dataPath,
    fixture.config.adminPassword,
...
test('failed session persistence never returns a login cookie and recovers after storage is repaired', async (t) => {
  ...
  fixture.onCleanup(() => db.close());
  db.exec(
    "CREATE TRIGGER reject_session BEFORE INSERT ON admin_sessions BEGIN SELECT RAISE(ABORT, 'synthetic failure'); END",
  );
  t.mock.method(console, 'error', () => undefined);
  const result = await fixture.request(
    '/api/login',
    post({ password: fixture.config.adminPassword }),
  );
  contract(result, 500, 'INTERNAL_ERROR', [
    fixture.config.adminPassword,
