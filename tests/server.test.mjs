...
const sessionSecret =
  'test-session-secret-that-is-longer-than-thirty-two-characters-2026';
...
function readStoredData() {
  ...
  try {
    return {
      events: createAnalyticsRepository(db).findAll(),
      inquiries: createInquiryRepository(db).findAll(),
    };
  } finally {
    db.close();
  }
...
test.before(async () => {
  temporaryDirectory = await mkdtemp(
    path.join(os.tmpdir(), 'jiuyue-production-test-'),
  );
  dataPath = path.join(temporaryDirectory, 'site.db');
  port = await availablePort();

  const now = new Date();
  boundaryTimestamp = new Date(
    Date.UTC(
      now.getUTCFullYear(),
      now.getUTCMonth(),
      now.getUTCDate() - 2,
...
test('完整生产接口、安全控制、隐私口径、时区、留存与会话流程', async () => {
  ...
  assert.equal(
    cleanedAtStartup.events.some((event) => event.id === 'expired-event'),
    false,
    '默认 180 天留存应在启动时清理过期事件',
  );
  assert.equal(
    cleanedAtStartup.events.some((event) => event.id === 'boundary-event'),
    true,
  );
  ...
  const media = await fetch(
    `http://127.0.0.1:${port}/assets/media/business-license.jpg`,
  );
  assert.equal(media.status, 200);
  assert.equal(media.headers.get('content-type'), 'image/jpeg');
  assert.ok((await media.arrayBuffer()).byteLength > 1000);
  assert.equal(
    (await fetch(`http://127.0.0.1:${port}/server.mjs`)).status,
    404,
  );
  assert.equal(
    (await fetch(`http://127.0.0.1:${port}/assets/media/%2e%2e%2findex.html`))
      .status,
    404,
  );
  ...
  result = await jsonRequest('/api/events', {
    ...
    body: JSON.stringify({
      ...
      referrer: 'https://ref.example/path?private=value',
      utm: { source: 'test-campaign' },
      device: 'desktop',
      ...analyticsConsent,
    }),
  });
  ...
  const rejectedAnalyticsInquiry = {
    ...
    preferredTime: '周末',
    privacyConsent: true,
    sourcePage: '/should-not-store',
    sourceSection: 'assessment',
    visitorId: 'visitor-should-not-store',
    referrer: 'https://ref.example/private?token=secret',
    utm: { source: 'should-not-store' },
  ...
  result = await jsonRequest('/api/inquiries', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify(rejectedAnalyticsInquiry),
  });
  ...
  result = await jsonRequest('/api/dashboard', {
  ...
  assert.equal(
    result.body.metrics.conversion,
    50,
    '转化率必须使用同一统计事件窗口的唯一访客口径',
  );
  const boundaryDay = result.body.daily.find(
    (day) => day.date === boundaryDate,
  );
  assert.ok(boundaryDay, `最近七天中缺少上海日期 ${boundaryDate}`);
  assert.equal(boundaryDay.visits, 1, 'UTC 16:30 应计入次日的上海日期');
  assert.equal(
    result.body.daily.reduce((sum, day) => sum + day.enquiries, 0),
    1,
  );
  assert.equal(
    result.body.daily.reduce((sum, day) => sum + day.trackedConversions, 0),
    1,
  );
  ...
  assert.equal(
    'visitorId' in result.body.inquiries[0],
    false,
    '实名询盘不得保存随机访客标识',
  );
  assert.equal('ip' in result.body.inquiries[0], false);
  assert.equal(
    result.body.inquiries[0].privacyNoticeVersion,
    analyticsNoticeVersion,
  );
  ...
  const storedPageView = storedBeforeRestart.events.find(
    (event) =>
      event.visitorId === 'visitor-test-1' && event.eventType === 'page_view',
  );
  ...
  for (let attempt = 1; attempt <= 6; attempt += 1) {
    result = await jsonRequest('/api/login', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ password: `wrong-value-${attempt}` }),
    });
  }
  assert.equal(result.response.status, 429);

  const weakGenerated = spawnSync(
    process.execPath,
    ['tools/generate-secrets.mjs', '--json'],
    {
      cwd: root,
  ...
  const brandGenerated = spawnSync(
    process.execPath,
    ['tools/generate-secrets.mjs', '--json'],
    {
      cwd: root,
      input: 'Jiuyue!Training7Secure',
      encoding: 'utf8',
    },
  );
  assert.notEqual(brandGenerated.status, 0, '初始化工具必须拒绝品牌相关口令');
  const phoneGenerated = spawnSync(
    process.execPath,
    ['tools/generate-secrets.mjs', '--json'],
    {
      cwd: root,
      input: 'River!180-6173-6378QuartzA',
      encoding: 'utf8',
    },
  );
  assert.notEqual(
    phoneGenerated.status,
    0,
    '初始化工具必须拒绝包含手机号的口令',
  );
  ...
  assert.match(
    generatedConfig.ADMIN_PASSWORD_HASH,
  ...
  await startServer({
    ADMIN_PASSWORD: '',
    ADMIN_PASSWORD_HASH: generatedConfig.ADMIN_PASSWORD_HASH,
    SESSION_SECRET: generatedConfig.SESSION_SECRET,
  });
  result = await jsonRequest('/api/login', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ password: hashPassword }),
  });
  assert.equal(
    result.response.status,
    200,
    '初始化工具生成的 scrypt 哈希必须可用于登录',
  );
});
