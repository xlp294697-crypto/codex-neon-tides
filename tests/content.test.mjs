...
function containsExif(buffer) {
  ...
  while (offset + 4 <= buffer.length) {
    ...
    if (marker === 0xe1 && buffer.subarray(offset + 4, offset + 10).toString('ascii') === 'Exif\0\0') return true;
    offset += 2 + length;
  }
  return false;
}
...
test('发布媒体清单与页面引用严格一致', async () => {
  ...
  assert.equal(new Set(manifest.map((item) => item.id)).size, manifest.length, '媒体 ID 必须唯一');
  assert.equal(new Set(manifest.map((item) => item.file)).size, manifest.length, '媒体文件名必须唯一');

  const actualFiles = (await readdir(mediaRoot)).sort();
  const expectedFiles = manifest.map((item) => item.file).sort();
  assert.deepEqual(actualFiles, expectedFiles, '媒体目录只能包含清单中的发布版图片');
  ...
  for (const item of manifest) {
    ...
    assert.ok(index.includes(`/assets/media/${item.file}`), `${item.file} 未被首页引用`);
    assert.ok(index.includes(`data-target-id="${item.id}"`), `${item.id} 缺少稳定统计标识`);
    const filePath = path.join(mediaRoot, item.file);
    const information = await lstat(filePath);
    assert.ok(information.isFile() && !information.isSymbolicLink(), `${item.file} 必须是普通文件`);
    const image = await readFile(filePath);
    assert.equal(image[0], 0xff);
    assert.equal(image[1], 0xd8);
    assert.equal(containsExif(image), false, `${item.file} 不应包含 EXIF 元数据`);
  }
});
...
test('首页包含预约、独立同意与正确联系方式', async () => {
  ...
  assert.ok(app.includes("const consentVersionKey = 'jiuyueAnalyticsConsentVersion'"));
  ...
  assert.ok(app.includes("sourcePage: analyticsAllowed ? location.pathname : ''"));
  assert.ok(app.includes('utm: analyticsAllowed ? campaignContext() : {}'));
  const bookingHandler = app.slice(app.indexOf("document.querySelector('#booking-form')"));
  assert.equal(bookingHandler.includes('visitorId:'), false, '具名预约请求不得携带随机访客标识');
});

test('隐私页与空数据种子存在', async () => {
  const privacy = await readFile(path.join(publicRoot, 'privacy.html'), 'utf8');
  for (const text of ['宜兴市氿悦体育发展有限公司', '18061736378', '未成年人信息', '去标识化访问统计', '180 天', '随机访客标识也不会写入具名预约记录', '删除']) {
    assert.ok(privacy.includes(text), `隐私页缺少：${text}`);
  }
  const gitignore = await readFile(path.join(root, '.gitignore'), 'utf8');
  assert.ok(gitignore.includes('data/*.json'), 'Git 忽略规则必须覆盖生产询盘数据');
