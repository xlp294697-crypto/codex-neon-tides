...
test('inquiry service persists only normalized appointment data and supports status and removal outcomes', async () => {
  const records = [];
  const repository = {
    async create(record) { records.unshift(record); },
  ...
  const value = validateInquiry({
    parentName: '虚构家长', phone: '000-000', grade: '测试年级', course: '测试课程',
    privacyConsent: true, visitorId: 'visitor-sensitive', ip: 'do-not-store',
  }).value;
  ...
  for (const field of ['privacyConsent', 'ip', 'visitorId']) assert.equal(field in records[0], false);
  assert.deepEqual(await service.updateStatus(record.id, 'invalid'), { ok: false, code: 'INVALID_INQUIRY_STATUS', message: '预约状态无效。' });
  assert.equal(records[0].status, 'New');
  assert.deepEqual(await service.updateStatus(record.id, 'Contacted'), { ok: true });
  ...
  const missing = { ok: false, code: 'INQUIRY_NOT_FOUND', message: '没有找到该预约。' };
...
test('failed inquiry persistence rejects creation instead of reporting success', async () => {
  const service = createInquiryService({ create() { throw new Error('unavailable'); } });
...
test('analytics service requeues failed batches and drains them once without losing event identity', async () => {
  ...
  }, { onWriteFailure() { unhealthy = true; } });
  service.setEventTimerControls({ schedule(delay) { delays.push(delay); }, cancel() {} });
  const value = validateEvent({
    analyticsConsent: true, analyticsNoticeVersion: '2026-08-22',
    analyticsConsentAt: '2026-01-01T00:00:00Z', page: '/', visitorId: 'visitor-test',
  }).value;
  assert.deepEqual(service.enqueue(value), { ok: true });
  assert.equal(service.eventBufferLength, 1);
  assert.equal(delays[0], 2000);
  await assert.rejects(service.flushEventBuffer({ drain: true }), /unavailable/);
...
test('analytics batches at 100 and rejects overflow after the pending batch plus 1000 buffered events', async () => {
  ...
  const batches = [];
  const service = createAnalyticsService({
    async insertBatch(batch) { await ready; batches.push(batch); },
  });
  for (let index = 0; index < 1100; index += 1) assert.equal(service.enqueue({ page: String(index) }).ok, true);
  assert.deepEqual(service.enqueue({ page: 'overflow' }), { ok: false, code: 'EVENT_QUEUE_FULL', message: '统计队列繁忙，请稍后再试。' });
  release();
  await service.flushEventBuffer({ drain: true });
