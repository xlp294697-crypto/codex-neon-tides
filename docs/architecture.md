...
目标生产拓扑为浏览器经 HTTPS 访问宿主机 Nginx；Nginx 负责 TLS、安全响应头和反向代理，Certbot 负责证书申请与续期；一个 Node.js 应用容器提供业务与静态资源；SQLite 保存业务数据。应用端口只绑定宿主机回环地址，不得直接暴露公网。SQLite 与此业务规模要求 Node.js 应用始终只运行一个实例。

```text
浏览器 → HTTPS → 宿主机 Nginx → 127.0.0.1:3002 → Node.js 单体应用 → SQLite
```
