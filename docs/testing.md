...
4. **部署与恢复测试**：验证最终 Docker 镜像健康检查、回环端口约束、全新数据库初始化、旧数据库迁移、备份与恢复、Nginx 配置约束以及预发布线上冒烟流程。
