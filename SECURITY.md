...
- Linux 程序代码应由 root 持有且对 `jiuyue` 服务账户只读；只有 `data` 与 `backups` 可写。应用 systemd unit 只放行 `data`；备份 unit 放行两者，因为 SQLite WAL 读取可能需要共享内存写入。不要授予应用/备份账户 Docker socket 权限。
...
随包 Nginx 示例使用 `$remote_addr` 覆盖该头。Docker 模式把应用端口只绑定到宿主机 `127.0.0.1`，宿主机 Nginx 是唯一公网入口。同机代理保持 `APP_BIND_IP=127.0.0.1`；另一台硬件代理只能绑定服务器私网 IP，并由主机防火墙把 3002 的来源限制为该代理。
...
- `tools/backup-sqlite.mjs` 仅在新备份验证成功后删除 90 天以前、且名称符合本包格式的本地日常备份对；部署者应按实际必要期限调整，并监控异地副本。`pre-restore-backups` 与发布备份由操作员按 90 天政策单独核实清理。异地副本必须加密，存储账户与服务器分离，配置 90 天生命周期并验证取回恢复。工具不自带加密或已配置的异地复制。
- `tools/restore-sqlite.mjs` 强制核对同名 `.sha256.txt`、完整性/外键、精确迁移兼容性及当前库安全备份，目标须停止写入并明确确认绝对路径；没有忽略哈希的开关。校验和不能证明可信来源，必须保护 sidecar 与备份的权限。恢复会话表会恢复未到期旧会话，应按事件风险清除会话或轮换签名密钥后才开放后台。响应删除请求时应删除在线记录、登记受影响的备份期限，并在灾难恢复后重新执行删除清单。

不要执行 `docker compose down -v`，也不要在没有已验证异地备份时清理 Docker 卷；这会删除询盘数据卷。Nginx 证书位于宿主机 `/etc/letsencrypt`，应纳入独立的受控配置备份。

## 日志

默认保留：应用/安全日志 30 天、固定错误码健康指标 30 天、非个人发布与恢复演练元数据 180 天；事故保全例外记录理由和到期删除日期。备份工具和监控只输出固定代码及生成的非个人文件名，不输出源/目标路径、SQLite 错误、HTTP 正文、凭据或记录。不要扩大诊断以打印异常对象或完整 `docker inspect`/展开后的 Compose 配置。监控平台接收告警需单独配置，其目的地密钥不得入库或写入命令行。

应用默认只记录启动、停止和内部错误，不把正常访问或 IP 写入应用日志。Nginx 示例关闭访问日志。如果业务需要启用安全访问日志：
