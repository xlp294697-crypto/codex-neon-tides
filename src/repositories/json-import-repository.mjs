...
export function importJsonRecords(
  db,
  data,
  { existingDb = db, dryRun = false } = {},
) {
  const inquiries = createInquiryRepository(db);
  const events = createAnalyticsRepository(db);
  const existingInquiries = existingDb
    ? createInquiryRepository(existingDb)
    : null;
  const existingEvents = existingDb
    ? createAnalyticsRepository(existingDb)
    : null;
  return transaction(db, () => {
    ...
    for (const [kind, records, repository, existing] of [
      ...
      for (const record of records) {
        if (seen.has(record.id) || existing?.has(record.id))
          counts.skipped += 1;
        else {
