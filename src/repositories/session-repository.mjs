export function createSessionRepository(db) {
  ...
  const save =
    db.prepare(`INSERT INTO admin_sessions (token_hash, csrf_token_hash, role, created_at, expires_at)
  ...
  return {
    ...
    deleteSession(tokenHash) {
      return Boolean(remove.run(tokenHash).changes);
    },
    prune(now = Date.now()) {
      return Number(expired.run(now).changes);
    },
  };
}
