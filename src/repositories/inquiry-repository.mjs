...
const columns = [
  ...
  ['referrer', 'referrer'],
  ['analytics_attributed', 'analyticsAttributed'],
...
function fromRow(row) {
  return Object.fromEntries(
    columns
      .filter(([column]) => column !== 'updated_at' || row.updated_at !== null)
      .map(([column, field]) => [
        field,
        column === 'analytics_attributed' ? row[column] === 1 : row[column],
      ]),
  );
}
...
export function createInquiryRepository(db) {
  const insert = db.prepare(
    `INSERT INTO inquiries (${columns.map(([column]) => column).join(', ')}) VALUES (${columns.map(() => '?').join(', ')})`,
  );
  const all = db.prepare(
    'SELECT * FROM inquiries ORDER BY created_at DESC, rowid DESC',
  );
  ...
  const overflow = db.prepare(
    'SELECT id FROM inquiries ORDER BY created_at DESC, rowid DESC LIMIT -1 OFFSET ?',
  );
  ...
  const repository = {
    ...
    updateStatus(id, status, updatedAt) {
      ...
      return transaction(db, () => {
        ...
        audit.create({
          ...
          inquiryId: id,
          createdAt: updatedAt,
          payload: { fromStatus: previous.status, toStatus: status },
        });
        return true;
      });
    },
    ...
    prune(maxRecords) {
      if (!Number.isSafeInteger(maxRecords) || maxRecords < 0)
        throw new Error('Invalid inquiry record limit');
      return transaction(db, () => {
